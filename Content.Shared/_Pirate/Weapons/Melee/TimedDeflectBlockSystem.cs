...
public sealed class TimedDeflectBlockSystem : EntitySystem
{
    ...
    {
        return _timing.CurTime <= block.DeflectWindowEnd + GetDeflectLagCompensation(defender, block);
    }

    private TimeSpan GetDeflectLagCompensation(EntityUid defender, TimedDeflectBlockComponent block)
    {
        if (!_net.IsServer ||
            block.DeflectLagCompensationMultiplier <= 0f ||
        ...
        var compensationSeconds = MathF.Min(
            ...
            session.Ping / 1000f * block.DeflectLagCompensationMultiplier);
